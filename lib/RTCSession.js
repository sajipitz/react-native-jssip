...
module.exports = class RTCSession extends EventEmitter
{
  ...
  sendDTMF(tones, options = {})
  {
    ...
    if (transportType === JsSIP_C.DTMF_TRANSPORT.RFC2833)
    {
      ...
      // }
      logger.error('SendDTMF fails if RFC2833 used');
    ...
    if (this._tones)
    {
      ...
      this._tones += tones;
      logger.debug('dtmf Tones already queued');
    ...
    function _sendDTMF()
    {
      ...
      else
      {
        ...
        logger.debug('dtmf sending');
        dtmf.send(tone, options);
        timeout = duration + interToneGap;
      }

      // Set timeout for the next tone.
      logger.debug('dtmf timeout waiting for next tone');
      // must have done: 
      // - import BackgroundTimer from 'react-native-background-timer';
      // - global.BackgroundTimer = BackgroundTimer;
      // in a different file!
      global.BackgroundTimer.setTimeout(_sendDTMF.bind(this), timeout);
    }
  }
